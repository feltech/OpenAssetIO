...
#
"""
@namespace openassetio.pluginSystem.PythonPluginSystemUIDelegatePlugin
...
class PythonPluginSystemUIDelegatePlugin(PythonPluginSystemPlugin):
    ...
    binding to a @ref asset_management_system "asset management
    system's" UI delegation plugin.

    It used by the dynamic plug-in discovery mechanism (@ref
    openassetio.ui.pluginSystem.PythonPluginSystem) to instantiate the
    main classes in an implementation.
    ...
    attribute called 'openassetioUIPlugin', that holds a class derived
    from this.
    """
    ...
    def identifier(cls):
        ...
        Generally, this should be the identifier used by the uiDelegate.
        The identifier should use only alpha-numeric characters and '.',
        '_' or '-'. For example:

            "org.openassetio.test.uiDelegate"

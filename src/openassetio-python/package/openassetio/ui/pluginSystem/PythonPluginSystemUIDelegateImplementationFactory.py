...
"""
@namespace
openassetio.pluginSystem.PythonPluginSystemUIDelegateImplementationFactory
A single-class module, providing the
PythonPluginSystemUIDelegateImplementationFactory class.
"""
...
class PythonPluginSystemUIDelegateImplementationFactory(UIDelegateImplementationFactoryInterface):
    """
    A Factory to manage @ref
    openassetio.pluginSystem.PythonPluginSystemUIDelegatePlugin derived
    plugins. Not usually used directly by a @ref host, which instead
    uses the @fqref{hostApi.UIDelegateFactory} "UIDelegateFactory".

    The factory loads plugins from entry points registered within the
    `openassetio.uiDelegate_plugin` entry point group, as well as
    ...
    openassetio.pluginSystem.PythonPluginSystemUIDelegatePlugin
    "PythonPluginSystemUIDelegatePlugin" based plugins. It uses the
    ...
    *PYTHONPATH** and doesn't need to be a subset of those paths. This
    ...
    @fqref{pluginSystem.CppPluginSystemUIDelegateImplementationFactory}
    ...
    ## The name of the UIDelegatePlugin entry point for entry point
    ## discovered plugins.
    kPackageEntryPointGroup = "openassetio.uiDelegate_plugin"
    ...
    def __init__(self, logger, paths=None, disableEntryPointsPlugins=None):
        ...
        super(PythonPluginSystemUIDelegateImplementationFactory, self).__init__(logger)

        self.__pluginSystem = None
    ...
    def __scan(self):
        """
        Scans for PythonPluginSystemUIDelegatePlugins, and registers them
        ...
        if self.__paths:
            self.__pluginSystem.scan(self.__paths)

        if self.__disableEntryPointsPlugins:
            self._logger.debug("Entry point based plugins are disabled")
        else:
            self.__pluginSystem.scan_entry_points(self.kPackageEntryPointGroup)

    def identifiers(self):
        """
        @return list, all identifiers known to the factory.
        @see @ref openassetio.pluginSystem.PythonPluginSystemUIDelegatePlugin
    ...
    def instantiate(self, identifier):
        ...
        @param identifier `str` The identifier of the UIDelegateInterface
        to instantiate.

        @returns openassetio.ui.managerApi.UIDelegateInterface

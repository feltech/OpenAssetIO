...
/**
 * UIDelegate Factories are responsible for instantiating classes that
 * derive from @fqref{managerApi.UIDelegateInterface} for use within a
 * host.
 *
 * UIDelegateImplementationFactoryInterface defines the abstract interface
...
class OPENASSETIO_UI_EXPORT UIDelegateImplementationFactoryInterface {
 public:
  ...
  virtual ~UIDelegateImplementationFactoryInterface() = 0;

  /**
   * All identifiers known to the factory.
   *
   * @see @fqref{managerApi.UIDelegateInterface.identifier}
   * "UIDelegateInterface.identifier"
   */
  [[nodiscard]] virtual Identifiers identifiers() = 0;

  /**
   * Creates an instance of the \fqref{managerApi.UIDelegateInterface}

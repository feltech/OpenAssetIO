...
class OPENASSETIO_UI_EXPORT UIDelegate final {
 public:
   ...
   * "UIDelegateFactory" to select which UIDelegate to initialize, and so can
   * be used in preferences etc to persist the chosen UIDelegate. The
   ...
   * UIDelegate is to re-initialize the manager with any updated settings

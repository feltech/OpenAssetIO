...
UIDelegatePtr UIDelegateFactory::defaultUIDelegateForInterface(
  ...
  if (!configPath) {
    ...
    // will call this by default, and handle a null return uiDelegate, vs
...
UIDelegatePtr UIDelegateFactory::defaultUIDelegateForInterface(
    ...
    const log::LoggerInterfacePtr& logger) {
  {
    Str msg = "Loading default uiDelegate config at '";
    msg += configPath;
    msg += "'";
    logger->log(log::LoggerInterface::Severity::kDebug, msg);
  }
  ...
  if (!std::filesystem::exists(configPath)) {
    Str msg = "Could not load default uiDelegate config from '";
    msg += configPath;
